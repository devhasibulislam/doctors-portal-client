...
const RequireAuth = ({ children }) => {
    ...
    if (user?.providerData[0]?.providerId === 'password' && !user?.emailVerified) {
        return (
            <div class="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 lg:w-1/2 md:w-1/2 w-full lg:px-0 md:px-0 px-4">
                <div class="max-w-xl p-8 text-center text-gray-800 shadow-xl lg:max-w-3xl rounded-3xl lg:p-12 mx-auto bg-gray-50">
                    <h3 class="text-2xl">Thanks for signing up for Doctors Portal!</h3>
                    {sending && <Loading />}
